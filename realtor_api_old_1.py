...
with open (r"data.csv", 'w') as f:
    ...
    writer_obj.writerow(["id", "bathrooms_total", "bedrooms", "stories_total", "building_type",
                        "agency_name", "agency_type", "property_type", "lng", "lat", "ownership_type",
                        "page_url", "timestamp", "province", "price"])
...
for lat in range(lat_min, lat_max):
    for lng in range(lng_min, lng_max):
        ...
        for page_num in range(1,51):
            payload = f"LatitudeMax={lat+1}&LongitudeMax={lng+1}&LatitudeMin={lat}&LongitudeMin={lng}&CurrentPage={page_num}&Sort=6-D&PropertyTypeGroupID=1&TransactionTypeId=2&PropertySearchTypeId=0&Currency=CAD&RecordsPerPage=120&ApplicationId=1&CultureId=1&Version=7.0"
            ...
            for result in results:
                ...
                # agency_designation = result["Individual"][0]["Organization"]["Designation"]
            ...
            print(f"lat:{lat} lng:{lng} Scraped page {page_num} successfully with {resnum}")
            resnum = 0
